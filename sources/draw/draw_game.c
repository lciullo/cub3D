...
/*   Updated: 2023/10/12 10:50:59 by cllovio          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_wall(int *y, t_raycasting *raycasting, int y_max, long size_wall)
{
	...
	while (*y <= y_max)
	{
		...
		{
			x_pixel_get = ((raycasting->data->collision_cor[1] % SQUARE_SIZE) * SIZE_IMG) / SQUARE_SIZE;
			y_pixel_get = ((*y - ((SIZE_Y >> 1) - (size_wall >> 1))) * SIZE_IMG) / (size_wall);
			color = my_mlx_pixel_get(&raycasting->data->N_texture, x_pixel_get, y_pixel_get);
		}
		else if (raycasting->data->horizontal == false)
		{
			x_pixel_get = ((raycasting->data->collision_cor[0] % SQUARE_SIZE) * SIZE_IMG) / SQUARE_SIZE;
			y_pixel_get = ((*y - ((SIZE_Y >> 1) - (size_wall >> 1))) * SIZE_IMG) / (size_wall);			
			color = my_mlx_pixel_get(&raycasting->data->N_texture, x_pixel_get, y_pixel_get);
		}
